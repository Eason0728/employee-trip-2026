...
- 員工欄位：單位/店別/姓名/出生/身分證/手機/地址/職別/自費金額/搭車地點/攜帶親友(0-5)/參與漆彈／水晶彈/飲食習慣/備註。身分證欄位有「外籍夥伴請填居留證統一證號」提示。
...
- **年齡閘門**：`famPbGate(i)` + `famPbAllowed(birth)`——漆彈滿 12 歲、水晶彈滿 6 歲、未滿 6 歲只能「參與其他加購活動」。年齡以 `TRIP_DATE`（2026-09-14+08:00）計算，生日當天算滿歲。改生日會重新閘門並清掉不合法的已選項。選項值常數：`PB_YES/PB_CRYSTAL/PB_NO`。
...
- **截止日**：`SIGNUP_DEADLINE`＝2026-07-24T00:00+08:00（7/23 整天可填）。過期隱藏表單、顯示 `#closedNotice`、送出也會擋。
...
- **它是寫死欄位清單的**：前端 payload 每次新增 key（例如 f1diet），後端就要同步改，否則新資料默默被丟掉（踩過兩次）。**流程：改前端 payload → 同步更新 `docs/apps-script.gs` → 把完整程式碼貼給使用者 → 提醒三步驟**：
  1. 貼上取代全部程式碼
  2. 若欄位增減：刪掉試算表的「報名登記V2」和「統計」分頁讓程式重建表頭（分頁存在就不會重建，欄位會錯位）
