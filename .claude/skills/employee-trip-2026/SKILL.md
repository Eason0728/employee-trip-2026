...
- 截止：前端 `SIGNUP_DEADLINE`＝2026-08-27T00:00+08:00（8/26 整天可填；歷次 7/27→8/6→8/9→8/10→8/26），後端同日期。**原問卷 index.html 維持 7/24 截止不動**——兩頁截止日各自獨立。
...
- 網址 `.../employee-trip-2026/guardian.html`；**未滿 18 歲者專用（含眷屬）**。名單 9 位（依出遊日 2026/9/14 計算）：同仁 6＝林宸妤／王禹婕／徐佑昕（小辛辣光復店）、洪愷昱（墨竹亭光復店）、周冠銘＋林亞韻（墨竹亭金山店）；眷屬 3＝郭品暄／郭柏揚（小辛辣光復店，隨行許雅筑）、張峮寧（央廚，隨行張祥茗），眷屬由隨行同仁代填、選項標註「眷屬·隨行者」方便辨識。名單寫死在 `trip-data/build_guardian.py` 的 `MINORS`（四元組：姓名/單位/店別/標註），**名單有變要手動改再重跑產生器**。
- 收：同仁姓名（下拉選單，自動帶單位店別）＋法代姓名/關係/身分證/手機/地址＋備註（共 6 必填）。關係選「其他」才顯示說明欄。**同仁本人的出生/身分證不收**（2026-08-12 Eason 指定移除，報名表已有）。
...
- 測試 `tests/test-guardian.js`（32 項）。
...
- **截止後唯讀查閱**（`showClosedReadonly()`）：過期後若該裝置 localStorage 有存過資料，直接把它畫進確認頁（`renderResultView(d)`，跟正常送出成功共用同一段渲染邏輯，改欄位只需要改一個地方），隱藏「修改資料，重新送出」按鈕、成功文字改成「僅供查閱，無法再修改」。純本機唯讀，跟正式資料（試算表）無關，也不會反過來寫回任何地方。沒存過資料的裝置维持原本「報名已截止」空白公告。
